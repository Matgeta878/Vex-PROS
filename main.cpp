...
// also only takes in the angle you want to turn to, not the angle to turn
 void turnIMU(double angle){
   ...
   while(fabs(error) > 5){
		 ...
		 pros::lcd::set_text(2, std::to_string(imu_sensor.get_heading()));
		 output = kP*error;
...
// }
 ...
 void translate(int units,int voltage){
   ...
   while(avgDriveEncoderValue() < abs(units)) {
     set_tank(voltage * direction + (angle - imu_sensor.get_heading()),voltage * direction - (angle - imu_sensor.get_heading()));
     pros::delay(10);
   }
   //Brief Brake
...
//x2 in inches // x1 in inches // angle in radians // arclength in inches
 void yikesCurve(double x1, double x2, double angle, double arclength){
     translate(40, x1);
		 double radiusRight = arclength/angle + 4; //inches
		 double radiusLeft = arclength/angle - 4; //inches
	...
	while (true) {
	  ...
	  if(abs(rightJoystick) < 10){
	    rightJoystick = 0;
 		...
 		set_tank(0.2*leftJoystick, 0.2*rightJoystick);
